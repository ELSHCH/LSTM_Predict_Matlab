function [t_f,Y_f_mean,Y_f_std,XTrain,YTrain]=LSTM_PATTERN_KF_QRF(time_begin,sampleS,bs,time_end,nVar,P_hours_f,t_down,X_down,...
...
for i=1:numWindows-1
    ...
    XTest={[xq(1:numFeatures,1:lengthWindow)]}; % explained parameters 
    for tr=1:1 
     net=resetState(net);   
...
for j=1:numWindows+1
  if (~isempty(find(td(1,j)<t_down(time_begin)+P_hours_f)))&&(~isempty(find(td(sampleS,j)>t_down(time_begin)+P_hours_f)))
    idWindow=j;
  end;  
end; 
base = datenum(1970,1,1);
% datestr((t_down(time_begin)+P_hours_f)/86400+base)
% datestr(td(1,idWindow+1)/86400+base)
% datestr(td(sampleS,idWindow+1)/86400+base)
'idWindow'
numWindows+1
idWindow
...
hold on
for i=1:numWindows  
   xd(1:sampleS,1:numFeatures,numWindows-(i-1))=X_down(1+lengthT-(i*sampleS-i*shiftS):lengthT-((i-1)*sampleS-(i-1)*shiftS),ind_predictors);
   xd_pr(1:sampleS,1:numResponses,numWindows-(i-1))=X_down(1+lengthT-(i*sampleS-i*shiftS):lengthT-((i-1)*sampleS-(i-1)*shiftS),ind_responses);
end;
for n_r=1:numWindows
...
%   if n_r>=2 
%    if n_r==numWindows  
%      plot(td(1:lengthWindow,n_r),xd(1:lengthWindow,1,n_r),'o');
%    else
%      plot(td(1:lengthWindow,n_r),xd(1:lengthWindow,1,n_r),'.');  
%    end;
%   end;
...
% X_exp=[xq_explain(1:nVar,1:lengthWindow)';Q_std ;R_std]';
 ...
 Q_std = covarianceStateTransitionPattern(xd_pr(1:lengthWindow,1:numResponses,n_r)',...
...
% end;
  if n_r == numWindows-1
   for i2=1:sampleS 
     for i1=1:numResponses
        xq(i1,i2)=xd_pr(i2,i1,numWindows); % add uncertainty for flexible predicton
     end;
   end;  
   R_std = sampleCovariance(xq',numResponses,lengthWindow); % covariance of time series 
   for i2=1:numResponses 
     R_d(1:lengthWindow,i2,numWindows)=R_std(i2,i2);
     Q_d(1:lengthWindow,i2,numWindows)=Q_std(i2,i2);
   end;
  end;
...
%         plot(td(1:lengthWindow,n_r),Y_f(:,b_r,1),'m');  
   ...
   if n_r==idWindow-1
    ...
    %  plot(td(1:lengthWindow,n_r+1),Y_f(:,b_r,1),'x');
      hold on
   end;   
    %   plot(td(1:lengthWindow,n_r+1),Y_f(:,b_r,1),'g');
    %   ylim([-1,1]);
 end; 
 end;
